...
var RUNTIME_PACKAGE = 'rml-runtime/es';
...
(0, _objectAssign2['default'])(MLTransformer.prototype, {
  isConditionalNode: function isConditionalNode(element) {
    var attrs = element.attribs || {};
    return this.IF_ATTR_NAME in attrs || this.ELIF_ATTR_NAME in attrs || this.ELSE_ATTR_NAME in attrs;
  },
  ...
  transform: function transform(done) {
    ...
    var code = this.code,
        ...
        templateRuntimeModule = this.templateRuntimeModule;
    ...
    var handler = new _domhandler2['default'](function (error, children) {
      ...
      header.push('import' + ' $iterate from \'' + templateRuntimeModule + '/iterate\';');
      if (strictDataMember === false) {
        header.push('import' + ' $getLooseDataMember from \'' + templateRuntimeModule + '/getLooseDataMember\';');
      }
      if (pure) {
        header.push('import' + ' $createReactPureComponentClass from \'' + templateRuntimeModule + '/createReactPureComponentClass\';');
      }
      ...
      var hasOwnTemplates = Object.keys(subTemplatesCode).length;
      if (hasOwnTemplates) {
      ...
      if (subTemplatesName.length) {
        header.push('const $templates = {');
        subTemplatesName.forEach(function (name) {
          _this2.pushHeaderCode(2, '...' + name + ',');
        });
        if (hasOwnTemplates) {
          _this2.pushHeaderCode(2, '...$ownTemplates,');
        }
        header.push('};');
      } else if (hasOwnTemplates) {
        header.push('const $templates = $ownTemplates;');
      }
      header.push(HEADER);
      _this2.pushHeaderCode(2, 'return (');
      _this2.pushCode(2, ');');
  ...
  throwParseError: function throwParseError(config, originalError) {
  ...
  processExpression: function processExpression(exp, config) {
    try {
      return (0, _expression.transformExpression)(exp, this.scope, (0, _objectAssign2['default'])({
  ...
  generateCodeForTag: function generateCodeForTag(node, level_) {
    ...
    if (tag === 'template') {
      if (attrs.is) {
        ...
        if (attrs.key) {
          var key = this.processExpression(attrs.key, {
            node: node,
            attrName: 'data'
          });
          if (data === 'undefined') {
    ...
    if (this.FOR_ATTR_NAME in attrs) {
      ...
      if (keyName && !(0, _expression.hasExpression)(keyName)) {
      ...
      this.pushCode(level, '$iterate(' + forExp + ', (' + itemName + ', ' + indexName + ') => {');
      level += 2;
